...
function App() {
  ...
  const fetchData = (e) => {
    ...
    async function fetch() {
      try {
        console.log(unit)
        const res = await Axios.get(`http://api.openweathermap.org/data/2.5/weather?q=${city}&units=${unit}&appid=${API_KEY}`)
        setData(res.data)
  ...
  return (
    ...
    <div className="mb-3">
      ...
      <div className="form-check form-check-inline mb-3" onChange={radioChange}>
        <input className="form-check-input" type="radio" name="inlineRadioOptions" id="inlineRadio1" value="celsius" defaultChecked={unit === "metric"}/>
        <label className="form-check-label mr-4" htmlFor="inlineRadio1">Celsius</label>

        <input className="form-check-input" type="radio" name="inlineRadioOptions" id="inlineRadio2" value="fahrenheit" defaultChecked={unit !== "metric"}/>
      ...
      <form className="form-inline">
        ...
        <div style={{minWidth:"360px"}}>
          ...
          {data !== undefined && 
            <h3> {data.main.temp} {unit === "metric" ? cUnicode : fUnicode}
              <span> (Feels like: {data!==undefined && data.main.feels_like}{unit === "metric" ? cUnicode : fUnicode})</span>
            </h3>
          }
